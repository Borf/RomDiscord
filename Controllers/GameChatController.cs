	...
	public class GameChatController : Controller
	{
        ...
        public async Task Get(int guildid)
        {
            ...
            if (HttpContext.WebSockets.IsWebSocketRequest)
            {
                ...
                await Echo(webSocket, guild, context, channel);
            }
            else
            {
        ...
        static async Task Echo(WebSocket webSocket, Guild guild, Context context, SocketTextChannel channel)
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    client.PostAsync("https://discordapp.com/api/webhooks/1082048936660967444/60n3-ykh0SMXKcgDe_Vd3aOV1v2bu0sgX0rDs2KSAj8rm8Jc_9vPEPY9GOqOP_p8Likt", content).Wait();
        ...
        public static async Task DiscordMessageReceived(SocketMessage arg)
        {
            ...
            await ws.SendAsync(System.Text.Encoding.UTF8.GetBytes(msg), WebSocketMessageType.Text, true, CancellationToken.None);
        }
    }
}
